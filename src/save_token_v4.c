...
/*   Updated: 2025/08/13 16:51:04 by mcuenca-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_strcmp(nd->token, "<<") == 0 && nd->quote_type == NO_QUOTE)
		return (HEREDOC);
	else if (ft_strcmp(nd->token, ">>") == 0 && nd->quote_type == NO_QUOTE)
		return (APPEND);
	else if (ft_strcmp(nd->token, "<") == 0 && nd->quote_type == NO_QUOTE)
		return (DIR_IN);
	else if (ft_strcmp(nd->token, ">") == 0 && nd->quote_type == NO_QUOTE)
		...
		return (PIPE);
	else if (nd->token[0] == '$')/*(ft_strcmp(nd->token, "$") == 0)
...
t_bool	between_matchchr(char *str, char match, char avoid)
{
	...
	while (str[i])
	{
		//if (str[i] == '\\' && str[i + 1] == match)
		if (str[i] == match && str[i - 1] != '\\')
			i++;	
...
int	std_char(t_list	**head, char *cmmd, int *sd)
{
	...
	{
		is_quote(cmmd, &quote_state, &sd[END], &i);
		if (sd[END] == -1)
			return (-1);
		i++;
	}
